...
class app(templates.app, cmd.Cmd):
    ...
    ###
    def start(self):
        self.log.info('Starting console UI for %s-%s ...' % (config.PROJ_NAME, config.PROJ_VERSION))
        self.py = {}
        ...
        self.prompt_mode = config.PROJ_NAME.lower()
        self.prompt_path = []
        self.opt = {}
    ...
    def _do_help_commands(self, app, cmds):
        head = '%-24s %-70s' % ("%s commands name" % app, "Description")
        print "\n%s" % head
        print '-' * len(head)
        for title in cmds:
