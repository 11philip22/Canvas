...
class app(object):
    ...
    ###
    def start_cli(self, args=None, histfile=d2sec_config.HISTORY):
        ...
        self.log_level = int(self.verb)
        log.set_level(self.log_level)
        #self.start(args=self.args)
        self.start()

    def start(self):
        self.log.info("Proxy usage: %s" % os.environ['%s_PROXY' % d2sec_config.PROJ_NAME])
    ...
    def cli_setup(self):
        ...
        #os.environ['%s_VERB' % config.PROJ_NAME] = str(self.opts.verb)
        #os.environ['%s_PROXY' % config.PROJ_NAME] = str((0, 1)[not bool(self.opts.proxy)])
...
class dsquare(object):
    '''Generic santrex client'''

    def __init__(self, *args, **kwargs):
        print "Santrex client init !"
