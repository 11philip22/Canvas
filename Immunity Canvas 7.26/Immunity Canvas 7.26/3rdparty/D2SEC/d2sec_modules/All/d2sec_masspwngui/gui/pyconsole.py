...
# Modified by DSquare Security, LLC, 2007-2009
...
class _ReadLine(object):
    ...
    class Output(object):
        def __init__(self, console, tag_name):
            ...
            self.textview   = console
            self.tag_name   = tag_name
            self.lock       = Lock()        
        def write(self, text):
            self.lock.acquire()
            pos = self.buffer.get_iter_at_mark(self.buffer.get_insert())
            if self.textview.interactmode == True:
                if self.tag_name == 'stderr':
                    self.lock.release()
                    return
                # we don't want a 'stdout' insert for interactmode
                # but we update the textview as if it were prompt
                # data ...
                # have to do_insert True to prevent loop on insert
                self.textview.do_insert = True
                self.buffer.insert_with_tags_by_name(pos, text, 'session_interact')
                self.textview.do_insert = False
            else:
                self.buffer.insert_with_tags_by_name(pos, text, self.tag_name)
            # wanna update the cursor on each write ...
            if self.textview.interactmode == True:
                iter = self.buffer.get_iter_at_mark(self.buffer.get_insert())
                iter.set_line_offset(len(text))
                last_line = text
                if '\n' in text:
                    # get the last line (for prompt trickery)
                    last_line = text.split('\n').pop()
                # ninja trickery ...
                self.textview.last_interact_offset = len(last_line)
                #file('/tmp/lol.txt', 'ab').write('XXX: %s (%d)\n' % (text, len(text)))
                # cursor gets moved in _commit ... we WANT the old line offset there
                self.buffer.place_cursor(iter)
                self.buffer.move_mark_by_name('cursor', iter)
            self.lock.release()
        # for stdout flushing emulation        
        def flush(self):
            # use locking to prevent invalidated text buffer iterators
            self.lock.acquire()
            # flush pending gtk events to update in realtime ...
            while gtk.events_pending():
                gtk.main_iteration(False)
            # update textview scroll
            self.textview.scroll_to_mark(self.textview.cursor, 0.2)
            self.lock.release()
    ...
    def __init__(self):
        ...
        self.buffer2 = self.get_buffer()
        self.buffer2.connect("insert-text", self.on_buf_insert)
        self.buffer2.connect("delete-range", self.on_buf_delete)
        self.buffer2.connect("mark-set", self.on_buf_mark_set)
        self.do_insert = False
        self.do_delete = False

        self.stdout_tag = self.buffer2.create_tag("stdout", foreground="black", background="white")
        self.stderr_tag = self.buffer2.create_tag("stderr", foreground="red", background="white")
        # non editable output for session interact ...        
        self.immutable_tag = self.buffer2.create_tag('session_interact',\
                                                     foreground='black',\
                                                     background='white',\
                                                     editable=False)
        ...
        self.cursor = self.buffer2.create_mark("cursor",
                                              self.buffer2.get_start_iter(),
                                              False)
        insert = self.buffer2.get_insert()
    ...
    def raw_input(self, ps=None):        
        ...
        iter = self.buffer2.get_iter_at_mark(self.buffer2.get_insert())

        if ps:
            self.freeze_undo()
            self.buffer2.insert(iter, self.ps)
            self.thaw_undo()
        ...
        if self.run_on_raw_input:
            ...
            self.run_on_raw_input = None
            self.buffer2.insert_at_cursor(run_now + '\n')
            
        if self.interactmode == True:
            self.did_raw_input = True
    ...
    def __get_cursor(self):
        return self.buffer2.get_iter_at_mark(self.cursor)
    ...
    def __get_text(self, start, end):
        return self.buffer2.get_text(start, end, False)

    def __move_cursor_to(self, iter):
        self.buffer2.place_cursor(iter)
        self.buffer2.move_mark_by_name("cursor", iter)
    ...
    def __print_completions(self, completions):
        ...
        iter = self.buffer2.get_end_iter()
        self.__move_cursor_to(iter)
        self.__insert(iter, "\n")
    ...
    def _get_line(self):
        ...
        line = self.buffer2.get_text(start, end, False)
        return line
    ...
    def _commit(self):
        ...
        text = self._get_line()
        if self.interactmode == True:
            if self.in_raw_input == True:
                # we set the line old offset to be right after prompt :>
                old_offset = self.last_interact_offset
                #file('/tmp/lol.txt', 'ab').write('old offset: %d (into: %s)\n' % (old_offset, text))
                # fix for when something throws an error and you're on a newline ...
                if old_offset > len(text):
                    old_offset = 0
                if old_offset == len(text):
                    # send newline on empty return pressed ...
                    text += '\n'
                self.interactbuf.append(text[old_offset:])
    ...
    # this feeds lines that are buffered from
    # the raw_input in session interact mode
    def isactive(self):
        if self.did_raw_input == False:
            self.raw_input()
        while gtk.events_pending():
            gtk.main_iteration(False)
...
class _Console(_ReadLine, console_cli.CommandLineInterface):
    ...
    def __start(self):
        ...
        if self.banner:
            iter = self.buffer2.get_start_iter()
            self.buffer2.insert_with_tags_by_name(iter, self.banner, "stdout")
            if not iter.starts_line():
                self.buffer2.insert(iter, "\n")
    ...
    def exec_command(self, command):
        if self._get_line():
            self._commit()
        self.buffer2.insert_at_cursor(command)
        self._commit()
