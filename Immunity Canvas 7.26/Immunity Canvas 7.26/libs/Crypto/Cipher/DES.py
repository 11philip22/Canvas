...
You can aslo run the pyDes.py file without and arguments to see a simple test.
...
def fiftysix_to_sixtyfour(key):
	"""
	Takes 7 bytes and returns the correctly parity-d 
	8 byte key string
	"""
	newbits=[]
	bits=binary_from_string(key)
	#now we have [1,0,1,0,1...]
	for i in xrange(len(bits)/7):
		working_on_bits=bits[:7]
		bitcount=count_bits(working_on_bits)
		if bitcount%2:
			working_on_bits+=["1"]
		else:
			working_on_bits+=["0"]
		newbits+=working_on_bits[:]
		bits=bits[7:]
	newkey=string_from_binary(newbits)
	return newkey
...
class des:
	...
	def __init__(self, key, mode=ECB, IV=None):
		...
		"""
		if len(key)==7:
			#we got a 56 bit key which we need to convert to a 64 bit key
			key=fiftysix_to_sixtyfour(key)
	...
	def getIV(self):
		...
		try:
			...
			print "Did you create a des object in CBC mode without an IV?"
			raise
...
def example_des():
	...
	d = des(deskey1,DES_CBC_MODE, IV="\0\0\0\0\0\0\0\0")
	manip="A"*128
	enc = d.encrypt(manip)
	#should be all zeros, but we don't check it - this is just to test if we cause an exception in above call
	iv=d.getIV()
	print "IV=%s"%repr(iv)
