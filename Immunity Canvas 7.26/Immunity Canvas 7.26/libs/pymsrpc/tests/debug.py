...
def dump_ndr(ndr_object, level=0, count=0):
    ...
    if isinstance(ndr_object, ndr_array_fixed) or isinstance(ndr_object, ndr_array_varying) or isinstance(ndr_object, ndr_array_conformant_varying):
        sys.stdout.write(gap)
        sys.stdout.write("type: %s\n" % (ndr_object))
    ...
    elif isinstance(ndr_object, ndr_array_conformant):
        sys.stdout.write(gap)
        sys.stdout.write("type: %s\n" % (ndr_object))
        ...
        # sys.stdout.write("count: %d\n" % (ndr_object.get_count()))
        # TODO
        dump_ndr(ndr_object.basetype, level=level)
